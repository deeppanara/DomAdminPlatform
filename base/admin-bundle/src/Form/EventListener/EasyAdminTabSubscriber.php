...
namespace DomBase\DomAdminBundle\Form\EventListener;
...
class EasyAdminTabSubscriber implements EventSubscriberInterface
{
    ...
    {
        $formTabs = $event->getForm()->getConfig()->getAttribute('domadmin_form_tabs');
        ...
        foreach ($event->getForm() as $child) {
            ...
            if (\count($errors) > 0) {
                $formTab = $child->getConfig()->getAttribute('domadmin_form_tab');

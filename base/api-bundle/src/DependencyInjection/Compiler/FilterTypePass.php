...
final class FilterTypePass implements CompilerPassInterface
{
    ...
    {
        // type guessers
        $guessers = $this->findAndSortTaggedServices('domapi.filter.type_guesser', $container);
        ...
        $typesMap = [];
        $servicesMap = [];
        foreach ($container->findTaggedServiceIds('domapi.filter.type', true) as $serviceId => $attributes) {
        ...
        $container->getDefinition('domapi.filter.registry')
        ...
        $container->getDefinition('domapi.filter.extension')
            ->replaceArgument(0, ServiceLocatorTagPass::register($container, $servicesMap));
        $formRegistry = $container->getDefinition('form.registry');
        $formRegistry->replaceArgument(0, \array_merge([new Reference('domapi.filter.extension')], $formRegistry->getArgument(0)));
    }
}

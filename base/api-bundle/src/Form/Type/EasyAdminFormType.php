...
namespace DomBase\DomApiBundle\Form\Type;

use ArrayObject;
use DomBase\DomApiBundle\Configuration\ConfigManager;
use DomBase\DomApiBundle\Form\EventListener\EasyAdminTabSubscriber;
use DomBase\DomApiBundle\Form\Type\Configurator\TypeConfiguratorInterface;
use DomBase\DomApiBundle\Form\Util\FormTypeHelper;
...
class EasyAdminFormType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        foreach ($entityProperties as $name => $metadata) {
            ...
            if (\in_array($formFieldType, ['domapi_group', EasyAdminGroupType::class])) {
            ...
            // applied to the form fields defined after it) and store its details
            // in a property to get them in form template
            if (\in_array($formFieldType, ['domapi_tab', EasyAdminTabType::class])) {
            ...
            if (0 === \strpos($metadata['property'], '_domapi_form_design_element_')) {
            ...
            $formField->setAttribute('domapi_form_tab', $currentFormTab);
            $formField->setAttribute('domapi_form_group', $currentFormGroup);

            $builder->add($formField);
        }

        $builder->setAttribute('domapi_form_tabs', $formTabs);
        $builder->setAttribute('domapi_form_groups', $formGroups);
    ...
    {
        $view->vars['domapi_form_tabs'] = $form->getConfig()->getAttribute('domapi_form_tabs');
        $view->vars['domapi_form_groups'] = $form->getConfig()->getAttribute('domapi_form_groups');
    }

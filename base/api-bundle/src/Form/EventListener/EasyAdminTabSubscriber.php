...
namespace DomBase\DomApiBundle\Form\EventListener;
...
class EasyAdminTabSubscriber implements EventSubscriberInterface
{
    ...
    {
        $formTabs = $event->getForm()->getConfig()->getAttribute('domapi_form_tabs');
        ...
        foreach ($event->getForm() as $child) {
            ...
            if (\count($errors) > 0) {
                $formTab = $child->getConfig()->getAttribute('domapi_form_tab');

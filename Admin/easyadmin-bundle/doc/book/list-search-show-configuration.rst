...
``EasyCorp\Bundle\DomAdminBundle\Form\Filter\FilterInterface``. This interface
...
``EasyCorp\Bundle\DomAdminBundle\Form\Filter\Filter`` class. Consider this
...
* ``@DomAdmin/default/layout.html.twig``, the common layout that decorates the
  rest of the main templates;
* ``@DomAdmin/default/show.html.twig``, renders the contents stored by a given
  entity;
* ``@DomAdmin/default/list.html.twig``, renders the entity listings and the
  search results page;
* ``@DomAdmin/default/paginator.html.twig``, renders the pagination of the
  ``list`` view.
...
are rendered with the ``@DomAdmin/default/field_string.html.twig`` template.
There are also other fragments to render special values, such as
``@DomAdmin/default/label_null.html.twig`` for ``null`` values. Check out the
...
``templates/bundles/DomAdminBundle/default/`` with the same path as the
...
.. code-block:: twig
    ...
    {# templates/bundles/DomAdminBundle/default/layout.html.twig #}

    {# DON'T DO THIS: it will cause an infinite loop #}
    {% extends '@DomAdmin/default/layout.html.twig' %}
...
.. code-block:: twig
    ...
    {% extends '@DomAdmin/default/list.html.twig' %}
